...
[![OKF](https://img.shields.io/badge/OKF-v0.3.4-green.svg)](https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing)

> **The first open-source memory system built on Google's Open Knowledge Format (OKF) for the Hermes agent ecosystem. v0.3.4 introduces `HermesOKFMemoryProvider` — a native Hermes Agent plugin with `MemoryProvider` ABC integration. `pip install hermes-okf` and Hermes auto-discovers it. `hermes okf search|list|show|snapshot|restore` CLI commands work out of the box.**
